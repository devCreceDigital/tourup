...
import type { ChatMessage, LeadFormData, MapMarker, MatchResult, PipelineStep } from "@/types"
...
function buildAsistenteIAUrl(path: string): string {
  const rawBase = process.env.NEXT_PUBLIC_DJANGO_API_URL ?? "http://localhost:8000"
  const base = rawBase.replace(/\/+$/, "")
  const apiBase = base.endsWith("/api") ? base : `${base}/api`
  return `${apiBase}/asistente-ia${path}`
}
...
export function useChatSession() {
  ...
  const initSession = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch(buildAsistenteIAUrl("/sessions/"), {
        method: "POST",
  ...
  const sendMessage = useCallback(
    async (content: string): Promise<boolean> => {
      ...
      try {
        const response = await fetch(buildAsistenteIAUrl(`/sessions/${sessionId}/message/`), {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const submitLead = useCallback(
    async (data: LeadFormData, match: MatchResult): Promise<string> => {
      ...
      const response = await fetch(buildAsistenteIAUrl("/leads/simple/"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          session_token: sessionToken,
