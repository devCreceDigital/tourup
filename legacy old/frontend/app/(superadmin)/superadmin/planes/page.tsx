    ...
    <div className="p-8">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-[#ede9f8] p-16 text-center text-gray-400">
          ...
          No hay planes. Créalos desde el Django Admin.
        </div>
      ) : (
        <div className="grid grid-cols-3 gap-5">
          {planes.map((p) => (
            ...
            >
              ...
              <a
                href={`/django-admin/planes/plan/${p.id}/change/`}
                target="_blank"
                className="mt-4 block text-center text-xs text-[#5B4FE8] hover:underline"
              >
                Editar en Django Admin →
              </a>
            </div>
          ))}
        </div>
