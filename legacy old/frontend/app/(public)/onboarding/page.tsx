...
export default function OnboardingPage() {
    ...
    try {
      ...
      const { data: tenant, error: te } = await supabase!
        ...
        .insert([{
          ...
          dominio: agencia.dominio || autoSlug(agencia.nombre),
          zona_horaria: "America/Lima",
          plan_id: agencia.plan_id,
          status: "activo",
        }])
        .select().single();
      if (te) throw te;

      await supabase!.from("perfiles").update({ tenant_id: tenant.id }).eq("id", user.id);
      setTenantId(tenant.id);
      localStorage.setItem("totem_nombre_agencia", agencia.nombre);

      await fetchDjango(`/tenants/${tenant.id}/onboarding/`, {
        method: "POST",
  ...
  const guardarIdentidad = async (tid: string) => {
    await fetchDjango(`/tenants/${tid}/onboarding/`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  const crearViajes = async (tid: string) => {
    for (const v of viajes) {
      ...
      const vRes = await fetchDjango("/viajes/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          nombre: v.nombre,
  ...
  const handleSiguiente = async () => {
    if (paso === 1) {
      ...
      setLoading(true);
      if (tenantId) {
        await new Promise(r => setTimeout(r, 1500)); // esperar sync Django
                ...
                <div key={i} className="bg-gray-50 rounded-2xl p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <input type="text" value={m.nombre}
                      onChange={e => { const arr = [...equipo]; arr[i] = { ...arr[i], nombre: e.target.value }; setEquipo(arr); }}
                    ...
                    <input type="text" value={m.rol}
                      onChange={e => { const arr = [...equipo]; arr[i] = { ...arr[i], rol: e.target.value }; setEquipo(arr); }}
                      className="border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#5B4FE8]"
                      placeholder="Rol: Guía, Director..." />
                  </div>
                  <input type="text" value={m.foto_url}
                    onChange={e => { const arr = [...equipo]; arr[i] = { ...arr[i], foto_url: e.target.value }; setEquipo(arr); }}
        ...
        {paso === 4 && (
          <div>
            ...
            <div className="p-8 space-y-5">
                  ...
                  <div className="relative h-28 overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-black/30 flex items-center justify-center gap-2">
                      {IMAGENES_VIAJE.map((url, j) => (
                        <button key={j} onClick={() => { const arr = [...viajes]; arr[i] = { ...arr[i], imagen_url: url }; setViajes(arr); }}
                  ...
                  <div className="p-4 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <input type="text" value={v.nombre}
                        onChange={e => { const arr = [...viajes]; arr[i] = { ...arr[i], nombre: e.target.value }; setViajes(arr); }}
                        className="border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#5B4FE8]"
                        placeholder="Nombre del viaje" />
                      <input type="text" value={v.destino}
                        onChange={e => { const arr = [...viajes]; arr[i] = { ...arr[i], destino: e.target.value }; setViajes(arr); }}
                        className="border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#5B4FE8]"
                        placeholder="Destino" />
                      <input type="text" value={v.precio}
                        onChange={e => { const arr = [...viajes]; arr[i] = { ...arr[i], precio: e.target.value }; setViajes(arr); }}
                        className="border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#5B4FE8]"
                        placeholder="Precio (S/.)" />
                      <input type="text" value={v.duracion}
                        onChange={e => { const arr = [...viajes]; arr[i] = { ...arr[i], duracion: e.target.value }; setViajes(arr); }}
                        className="border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#5B4FE8]"
                        placeholder="Duración (ej: 5 días)" />
                    </div>
                    <textarea value={v.descripcion}
                      onChange={e => { const arr = [...viajes]; arr[i] = { ...arr[i], descripcion: e.target.value }; setViajes(arr); }}
              ...
              {viajes.length < 3 && (
                <button onClick={() => setViajes([...viajes, { nombre: "", destino: "", precio: "", duracion: "", imagen_url: IMAGENES_VIAJE[viajes.length % 4], descripcion: "" }])}
                  className="w-full py-3 border-2 border-dashed border-gray-200 rounded-2xl text-sm text-gray-400 hover:border-[#5B4FE8] hover:text-[#5B4FE8] transition-all flex items-center justify-center gap-2">
