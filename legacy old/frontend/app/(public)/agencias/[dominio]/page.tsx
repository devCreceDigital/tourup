...
export default function PaginaAgenciaPage() {
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/api/agencias/${dominio}/`)
      .then(r => r.json())
      .then(d => { setAgencia(d.agencia); setViajes(d.viajes ?? []); })
      .catch(() => setError(true))
  ...
  const getPrecio = (v: Viaje) => {
    ...
    if (pd?.length) { const d = pd.find(p => p.destacado) ?? pd[0]; return `${v.moneda === "PEN" ? "S/." : "$"} ${d.monto.toLocaleString()}`; }
