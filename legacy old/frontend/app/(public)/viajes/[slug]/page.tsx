...
export default function ViajePublicoPage() {
  ...
  useEffect(() => {
    // Buscar viaje por slug en el endpoint público
    fetch(`http://localhost:8000/api/public/viajes/${slug}/`)
      .then(r => r.json())
      .then(d => {
        const v = Array.isArray(d) ? d[0] : d.results?.[0] ?? d;
        if (!v?.id) { setError(true); return; }
        setViaje(v);
        // Cargar agencia via tenant_id
        if (v.tenant_id) {
          fetch(`http://localhost:8000/api/agencias/by-tenant/${v.tenant_id}/`)
            .then(r => r.json())
            .then(a => setAgencia(a.agencia ?? a))
            .catch(() => {});
        }
      })
      .catch(() => setError(true))
