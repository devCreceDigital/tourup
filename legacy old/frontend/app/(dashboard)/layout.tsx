...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (rol && rol !== "superadmin") {
      ...
      if (token) {
        fetchDjango("/perfil/me/").then(r => r.json()).then(async perfil => {
          if (perfil.tenant_id) {
            const r = await fetchDjango("/tenants/" + perfil.tenant_id + "/onboarding/");
            if (r.ok) {
              const t = await r.json();
              if (t.nombre) { setNombreAgencia(t.nombre); localStorage.setItem("totem_nombre_agencia", t.nombre); }
              if (t.logo_url) setLogoAgencia(t.logo_url);
              if (t.dominio) setDominio(t.dominio);
              if (t.color_primario) setColorPrimario(t.color_primario);
            }
          }
        }).catch(() => {});
      }
    ...
    if (!rol) {
      ...
      if (token) {
        fetchDjango("/perfil/me/").then(r => r.json()).then(async perfil => {
          ...
          localStorage.setItem("totem_email", perfil.email || "");
          if (perfil.rol === "superadmin") { router.replace("/superadmin"); return; }
          setUserName(perfil.nombre || "Admin");
          setUserEmail(perfil.email || "");
          setUserInitials((perfil.nombre || "AD").substring(0,2).toUpperCase());
          if (perfil.tenant_id) {
            const r = await fetchDjango("/tenants/" + perfil.tenant_id + "/onboarding/");
            if (r.ok) {
              const t = await r.json();
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const me = await (await fetchDjango("/perfil/me/")).json();
        if (me.nombre) { setUserName(me.nombre); localStorage.setItem("totem_nombre", me.nombre); setUserInitials(me.nombre.substring(0,2).toUpperCase()); }
        if (me.rol) { localStorage.setItem("totem_rol", me.rol); }
        if (me.email) { localStorage.setItem("totem_email", me.email); }
        if (!me.tenant_id) return;
        const cfg = await (await fetchDjango(`/tenants/${me.tenant_id}/onboarding/`)).json();
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        const res = await fetchDjango("/notificaciones/?page=1&page_size=5");
        if (!res.ok) return;
        ...
        const unreadRes = await fetchDjango("/notificaciones/?unread=1&page=1&page_size=1");
        if (unreadRes.ok) {
  ...
  const handleLogout = async () => {
    ...
    localStorage.removeItem("totem_rol");
    localStorage.removeItem("totem_email");
    localStorage.removeItem("totem_nombre");
    localStorage.removeItem("totem_nombre_agencia");
    document.cookie = "totem_token=; path=/; max-age=0";
