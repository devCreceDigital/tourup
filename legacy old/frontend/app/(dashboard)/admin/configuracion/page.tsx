...
export default function AdminConfiguracionPage() {
  ...
  const loadPrefs = useCallback(async () => {
    setLoading(true);
    try {
      const resMe = await fetchDjango("/perfil/me/");
      if (!resMe.ok) return;
      const me = await resMe.json();
      const tid = me.tenant_id;
      if (!tid) return;
      setTenantId(tid);
      const resPref = await fetchDjango(`/tenants/${tid}/preferencias/`);
      if (resPref.ok) {
        const data = await resPref.json();
        setPrefs({ ...DEFAULT_PREFS, ...(data.preferencias ?? {}) });
      }
    } finally {
      setLoading(false);
    }
  ...
  const toggle = async (key: keyof Prefs) => {
    ...
    setSaving(true);
    try {
      await fetchDjango(`/tenants/${tenantId}/preferencias/`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ preferencias: next }),
      });
      setSaved(true);
