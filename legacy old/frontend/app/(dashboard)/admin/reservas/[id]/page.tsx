...
const ESTADO_CONFIG: Record<string, { bg: string; text: string; icon: any; label: string }> = {
...
export default function InscripcionDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetchDjango(`/inscripciones/${id}/`);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-[#E0E4EF] px-6 py-4 flex items-center justify-between shadow-[0_4px_24px_rgba(91,79,232,0.08)]">
        ...
        {insc && (() => {
          const cfg = ESTADO_CONFIG[insc.estado] ?? { bg: "bg-[#f5f3fb]", text: "text-[#666]", label: insc.estado };
          const Icon = cfg.icon;
          return (
