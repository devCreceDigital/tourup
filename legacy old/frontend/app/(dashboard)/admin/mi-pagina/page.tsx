...
export default function MiPaginaPage() {
  ...
  useEffect(() => {
    fetchDjango("/perfil/me/")
      .then(r => r.json())
      .then(async me => {
        setTenantId(me.tenant_id ?? "");
        // nombre se carga desde el tenant
        if (me.tenant_id) {
          const res = await fetchDjango(`/tenants/${me.tenant_id}/onboarding/`);
          if (res.ok) {
            const d = await res.json();
            setDominio(d.dominio ?? "");
            setNombre(d.nombre ?? "");
            fetchDjango("/viajes/").then(r => r.json()).then(dv => setViajes(Array.isArray(dv) ? dv : dv.results ?? []));
            setConfig(prev => ({
              ...prev,
              logo_url:        d.logo_url ?? "",
              banner_url:      d.banner_url || BANNERS[0],
              slogan:          d.slogan ?? "",
              descripcion:     d.descripcion ?? "",
              color_primario:  d.color_primario ?? "#5B4FE8",
              color_secundario:d.color_secundario ?? "#1a1a2e",
              website:         d.website ?? "",
              telefono:        d.telefono ?? "",
              dominio_custom:  d.dominio_custom ?? "",
              redes_sociales:  d.redes_sociales ?? {},
            }));
            setCardStyle(d.preferencias?.card_style ?? 'clasica');
            setConfig(prev => ({
              ...prev,
            }));
          }
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
    const r1 = await fetchDjango(`/tenants/${tenantId}/onboarding/`, {
      method: "PATCH",
            ...
            <>
              {[
                ...
                <div key={key}>
                  ...
                  <input type="text"
                    value={(config.redes_sociales as any)[key] ?? ""}
          ...
          <div className="p-6 bg-[#f8f7ff]">
            ...
            <div className={cardStyle === 'spotlight' ? 'grid grid-cols-3 gap-3 auto-rows-auto' : 'grid grid-cols-3 gap-3'}>
              {viajes.length === 0 ? (
                ...
                const precio = v.configuracion?.precios_detalle?.[0]?.monto ? (v.moneda === "PEN" ? "S/. " : "$ ") + v.configuracion.precios_detalle[0].monto : "";
