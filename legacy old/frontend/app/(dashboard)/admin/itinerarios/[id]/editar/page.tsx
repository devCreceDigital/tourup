...
import { DiasPanel } from "@/components/admin/itinerarios/DiasPanel";
import { DiaEditor } from "@/components/admin/itinerarios/DiaEditor";
import { fetchDjango } from "@/lib/api";
import type { Itinerario, DiaItinerario, EventoItinerario, Actividad } from "@/types";
...
export default function ItinerarioEditorPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [resIt, resActs] = await Promise.all([
        fetchDjango(`/itinerarios/${id}/`),
        fetchDjango("/actividades"),
      ]);
      if (resIt.ok) {
        ...
        setDias(diasData);
        if (diasData.length > 0) setDiaActivoId(diasData[0].id);
      }
      if (resActs.ok) {
  ...
  const handleSaveHeader = async () => {
    ...
    try {
      const res = await fetchDjango(`/itinerarios/${id}`, {
        method: "PATCH",
  ...
  const handleClonar = async () => {
    ...
    setCloningIt(true);
    try {
      const res = await fetchDjango(`/itinerarios/${id}/clonar/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
      if (res.ok) {
  ...
  const handleAgregarDia = async () => {
    ...
    try {
      const res = await fetchDjango(`/itinerarios/${id}/dias/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleEliminarDia = async (diaId: string) => {
    ...
    if (!ok) return;
    try {
      const res = await fetchDjango(`/itinerarios/${id}/dias/${diaId}/`, { method: "DELETE" });
  ...
  const handleMoverDia = async (diaId: string, direction: "up" | "down") => {
    ...
    try {
      await Promise.all([
        fetchDjango(`/itinerarios/${id}/dias/${renumbered[idx].id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ numero_dia: renumbered[idx].numero_dia }),
        }),
        fetchDjango(`/itinerarios/${id}/dias/${renumbered[swapIdx].id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ numero_dia: renumbered[swapIdx].numero_dia }),
        }),
      ]);
    } catch (err) {
      console.error(err);
  ...
  const handleUpdateDia = async (diaId: string, changes: Partial<DiaItinerario>) => {
    ...
    try {
      const res = await fetchDjango(`/itinerarios/${id}/dias/${diaId}/`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleAddEvento = async (diaId: string) => {
    ...
    if (!dia) return;
    try {
      const res = await fetchDjango(`/itinerarios/${id}/dias/${diaId}/eventos/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleUpdateEvento = async (diaId: string, eventoId: string, changes: Partial<EventoItinerario>) => {
    try {
      const res = await fetchDjango(`/itinerarios/${id}/dias/${diaId}/eventos/${eventoId}/`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(changes),
      });
  ...
  const handleDeleteEvento = async (diaId: string, eventoId: string) => {
    try {
      const res = await fetchDjango(`/itinerarios/${id}/dias/${diaId}/eventos/${eventoId}/`, { method: "DELETE" });
      if (res.ok || res.status === 204) {
        setDias((prev) => prev.map((d) =>
  ...
  const handleMoverEvento = async (diaId: string, eventoId: string, direction: "up" | "down") => {
    ...
    [eventos[idx], eventos[swapIdx]] = [eventos[swapIdx], eventos[idx]];
    const reordered = eventos.map((ev, i) => ({ ...ev, orden: i }));
    setDias((prev) => prev.map((d) => d.id === diaId ? { ...d, eventos: reordered } : d));

    try {
      await Promise.all([
        fetchDjango(`/itinerarios/${id}/dias/${diaId}/eventos/${reordered[idx].id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ orden: reordered[idx].orden }),
        }),
        fetchDjango(`/itinerarios/${id}/dias/${diaId}/eventos/${reordered[swapIdx].id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ orden: reordered[swapIdx].orden }),
        }),
      ]);
    } catch (err) {
      console.error(err);
