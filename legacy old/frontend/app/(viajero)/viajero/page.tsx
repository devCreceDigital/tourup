...
export default function ViajeroDashboardPage() {
  ...
  const handleCancelarInscripcion = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetchDjango(`/inscripciones/${inscripcion.id}/`, {
        method: "DELETE",
      });
      if (res.ok) {
        setInscripcion(null);
    ...
    fetchDjango("/mis-inscripciones/")
      ...
      .then(async (data) => {
        ...
        if (results.length > 0) {
          ...
          const viajeRes = await fetchDjango(`/viajes/${primera.viaje}/`);
          const viajeData: ViajeDetalle = await viajeRes.json();
          setViaje(viajeData);
          if (viajeData.itinerario_id) {
            const itinRes = await fetchDjango(`/itinerarios/${viajeData.itinerario_id}/`);
