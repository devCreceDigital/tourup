...
import FiltrosInscritos from "@/components/admin/inscripciones/FiltrosInscritos";
import TablaInscritos from "@/components/admin/inscripciones/TablaInscritos";
import DrawerDetalleInscripcion from "@/components/admin/inscripciones/DrawerDetalleInscripcion";
...
export default function InscripcionesClient({ viajeId }: { viajeId: string }) {
  ...
  const loadInscritos = useCallback(async () => {
    try {
      const res = await fetchDjango(`/viajes/${viajeId}/inscripciones/`);
  ...
  const handleNuevaInscripcion = async () => {
    ...
    try {
      const res = await fetchDjango(`/viajes/${viajeId}/inscripciones/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: nuevoEmail.trim(), nombre_completo: nuevoNombre.trim() }),
      });
      if (res.ok) {
        setShowModal(false);
        setNuevoEmail("");
