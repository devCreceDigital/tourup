...
import type { BudgetResult, ChatMessage, ItineraryDay, MatchResult, WeatherResult } from "@/types"
import { CalendarDays, Flag, Layers, Map, Plus, Wallet } from "lucide-react"
import TripMap from "@/components/shared/TripMap"
import { resolveDestinations, type GeoPoint } from "@/lib/geoHelper"
...
interface WorkspacePanelProps {
  ...
  preloadedTrip?: PreloadedTrip | null
}
...
export default function WorkspacePanel({
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/asistente-ia/trips/`, {
      method: "POST",
      ...
      .then((r) => r.json())
      .then((data) => {
        if (data.share_token) setAutoShareUrl(`/trip?token=${data.share_token}`)
      })
    ...
    <section className={styles.aiWorkspacePanel} aria-label="Workspace">
      ...
      {hasItinerary && (
        <div style={{padding: "6px 12px", borderBottom: "0.5px solid #e5e7eb", display: "flex", justifyContent: "flex-end", gap: 8}}>
          {autoShareUrl && (
            <PdfButton shareToken={autoShareUrl.split("token=")[1]} />
          )}
          <ShareButton
...
function ShareButton({ sessionToken, itinerary, budget, weather, destination, days, preloadedUrl }: ShareButtonProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      const apiBase = getApiBaseUrl()
      const res = await fetch(`${apiBase}/asistente-ia/trips/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          session_token: sessionToken,
      ...
      console.log("Share response status:", res.status, res.headers.get("content-type"))
      const data = await res.json()
      const url = `/trip?token=${data.share_token}`
      setShareUrl(url)
...
function PdfButton({ shareToken }: { shareToken: string }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      const apiBase = getApiBaseUrl()
      const res = await fetch(`${apiBase}/asistente-ia/trips/${shareToken}/pdf/`)
