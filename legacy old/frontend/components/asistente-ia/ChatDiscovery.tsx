...
export default function ChatDiscovery({ cloneToken }: { cloneToken?: string | null }) {
  ...
  const [preloadedTrip, setPreloadedTrip] = useState<any | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/asistente-ia/trips/${cloneToken}/`)
      .then(r => r.json())
      .then(data => {
        if (data.itinerary) setPreloadedTrip(data)
      })
      .catch(e => console.error("Clone fetch error:", e))
  }, [cloneToken])
