...
export default function ViajeEditarAvanzadoClient({ viajeId, nombre, viaje }: Props) {
  ...
  const handleEnviarSoporte = async () => {
    ...
    try {
      const res = await requestTotemApi("/support/tickets", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          subject: soporteForm.asunto,
          description: soporteForm.descripcion,
          priority: soporteForm.prioridad,
          metadata: { tripId: viajeId },
        }),
      });
      if (res.ok) {
