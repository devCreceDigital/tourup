...
export function resolveRoleFromUser(user: AppUser): AppRole | null {
  if (!user) return null;
...
export function isAdminOrSuperAdmin(user: AppUser): boolean {
  const rol = resolveRoleFromUser(user);
  return rol === "admin" || rol === "superadmin";
}

export function isViajero(user: AppUser): boolean {
  return resolveRoleFromUser(user) === "viajero";
}

export function getDashboardPath(user: AppUser): string {
  const rol = resolveRoleFromUser(user);
