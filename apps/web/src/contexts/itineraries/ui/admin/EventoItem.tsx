...
import type { EventoItinerario, Actividad } from "@/shared/domain/totem-types";
...
export function EventoItem({ evento, index, total, actividades, onUpdate, onDelete, onMover }: Props) {
  ...
  const handleSave = () => {
    const changes: Partial<EventoItinerario> = {
      descripcion: draft.descripcion,
      tipo: draft.tipo,
    };
    if (draft.hora_inicio !== undefined) changes.hora_inicio = draft.hora_inicio;
    if (draft.hora_fin !== undefined) changes.hora_fin = draft.hora_fin;
    if (draft.actividad_id !== undefined) changes.actividad_id = draft.actividad_id;
    onUpdate(evento.id, changes);
    setEditando(false);
  };
