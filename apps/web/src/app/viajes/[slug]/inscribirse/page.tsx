import WizardInscripcion from "@/contexts/enrollments/ui/public/WizardInscripcion";
import Link from "next/link";
import { ArrowLeft } from "lucide-react";
import { fetchTotemServer } from "@/shared/server/fetch";

async function fetchViajeBasico(slug: string): Promise<{ nombre: string; precioBase: number }> {
  try {
    const res = await fetchTotemServer(`/trips/public/${encodeURIComponent(slug)}`);
    if (res.ok) {
