...
export default function ViajeroNotificacionesPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await requestTotemApi("/notifications/");
      if (!res.ok) {
        setItems([]);
        return;
      }
  ...
  const marcar = async (ids: string[], leida: boolean) => {
    await requestTotemApi("/notifications/mark-read", {
      method: "POST",
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href="/viajero/documentos">
            <Button variant="outline">Ir a Documentos</Button>
          </Link>
          <Link href="/viajero/pagos">
            <Button className="bg-[#00B4FC] hover:bg-[#0098d6]">Ir a Pagos</Button>
          </Link>
        </div>
      </div>
