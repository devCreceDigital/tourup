...
export default function ViajeEditorPage() {
  ...
  const [form, setForm] = useState<ViajeEditorForm>({
  ...
  const set = <K extends keyof ViajeEditorForm>(k: K, v: ViajeEditorForm[K]) => setForm(p => ({ ...p, [k]: v }));

  useEffect(() => {
    requestTotemApi(`/trips/${id}/`)
      .then(r => r.json())
      .then(d => {
  ...
  const handleSave = async (overrides?: Partial<typeof form>) => {
    ...
    setSaving(true);
    await requestTotemApi(`/trips/${id}/`, {
      method: "PATCH",
                      ...
                      <div key={p.id} className={`flex items-center gap-3 p-3 rounded-xl border-2 ${p.destacado ? "border-[#5B4FE8] bg-[#5B4FE8]/5" : "border-[#ede9f8]"}`}>
                        <div className="flex-1 grid grid-cols-2 gap-2">
                          <input type="text" value={p.nombre}
                            onChange={e => {
                              const current = form.precios_detalle[i];
                              if (current === undefined) return;
                              const arr = [...form.precios_detalle];
                              arr[i] = { ...current, nombre: e.target.value };
                              set("precios_detalle", arr);
                            }}
                          ...
                          <input type="number" value={p.monto} min={0}
                            onChange={e => {
                              ...
                              if (current === undefined) return;
                              const arr = [...form.precios_detalle];
                              arr[i] = { ...current, monto: Number(e.target.value) };
                              set("precios_detalle", arr);
                            }}
                            className="border border-[#ede9f8] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#5B4FE8]"
                            placeholder="Monto" />
                        ...
                        <button onClick={() => {
                            ...
                            if (current === undefined) return;
                            const arr = [...form.precios_detalle];
                            arr[i] = { ...current, destacado: !current.destacado };
                            set("precios_detalle", arr);
                          }}
