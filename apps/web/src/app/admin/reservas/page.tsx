...
export default function ReservasPage() {
  ...
  const loadViajes = useCallback(async () => {
    const res = await requestTotemApi("/trips/?page_size=100");
    if (res.ok) {
  ...
  const handleEliminar = async (id: string, nombre: string) => {
    ...
    try {
      const res = await requestTotemApi(`/enrollments/${id}/`, { method: "DELETE" });
  ...
  const handleCrearInscripcion = async () => {
    ...
    setErrorInsc(null);
    try {
      const res = await requestTotemApi(`/enrollments?tripId=`, {
        method: "POST",
    ...
    const t = setTimeout(() => {
      void (async () => {
        try {
          ...
          const res = await requestTotemApi(`/enrollments/?${qs.toString()}`);
          if (!res.ok) {
            setInscripciones([]);
            return;
          }
