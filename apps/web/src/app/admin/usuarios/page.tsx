...
const DEFAULT_ROL_BADGE = ROL_BADGE.usuario ?? { bg: "bg-[#f1efe8]", text: "text-[#5f5e5a]", label: "Usuario" };
...
export default function UsuariosPage() {
  ...
  const loadData = useCallback(async (retries = 3) => {
    ...
    try {
      const [resU, resI] = await Promise.all([
        requestTotemApi("/identity/users"),
        requestTotemApi("/identity/invitations"),
      ]);
  ...
  const handleInvitar = async () => {
    ...
    setInvMsg(null);
    try {
      const res = await requestTotemApi("/identity/invitations", {
        method: "POST",
  ...
  const handleCambiarRol = async (id: string, nuevoRol: string) => {
    setCambiandoRol(id);
    try {
      const res = await requestTotemApi(`/identity/profiles//role`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        ...
        <div className="bg-white rounded-xl border border-[#E0E4EF] overflow-hidden shadow-[0_4px_24px_rgba(91,79,232,0.08)]">
          ...
          {usuarios.length === 0 ? (
            ...
            <table className="w-full text-[12px]">
              ...
              <tbody>
                {usuarios.map(u => {
                  const badge = ROL_BADGE[u.rol] ?? DEFAULT_ROL_BADGE;
                  return (
            ...
            <tbody>
              {invPendientes.map(inv => (
                <tr key={inv.id} className="border-b border-[#f5f3fb] hover:bg-[#faf9ff] transition">
                  ...
                  <td className="px-5 py-3">
                    <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold ${(ROL_BADGE[inv.rol] ?? DEFAULT_ROL_BADGE).bg} ${(ROL_BADGE[inv.rol] ?? DEFAULT_ROL_BADGE).text}`}>
                      {inv.rol}
                    </span>
                  </td>
