...
import { getLevelLabel, scoreToLevel } from "../lib/levels.js";
...
router.get("/operators", async (req, res) => {
  try {
    ...
    let rows: typeof operatorsTable.$inferSelect[];
    let total: number;

    if (sort_by === "diversified") {
      // Round-robin: fetch top N from each level, then interleave
      const LEVELS_ORDER = ["elite", "premium", "advanced", "growing", "emerging", "risk"];
      const perLevel = Math.ceil(limitNum / LEVELS_ORDER.length) + 1;
      const levelOffset = Math.floor(offset / LEVELS_ORDER.length);

      const levelCond = (lvl: string) => {
        const lc = eq(operatorsTable.level, lvl);
        return conditions.length > 0 ? and(lc, ...conditions) : lc;
      };

      const [countRes, ...levelResults] = await Promise.all([
        db.select({ count: sql<number>`count(*)` }).from(operatorsTable).where(where),
      ...
      const interleaved: typeof operatorsTable.$inferSelect[] = [];
      const maxLen = Math.max(...levelResults.map((r) => r.length));
      for (let i = 0; i < maxLen && interleaved.length < limitNum; i++) {
        for (const levelData of levelResults) {
          if (i < levelData.length && interleaved.length < limitNum) {
            interleaved.push(levelData[i]!);
          }
        }
      }
      ...
      const [dataRes, countRes] = await Promise.all([
        db.select().from(operatorsTable).where(where).orderBy(orderCol).limit(limitNum).offset(offset),
        db.select({ count: sql<number>`count(*)` }).from(operatorsTable).where(where),
      ]);
      rows = dataRes;
      total = Number(countRes[0]?.count ?? 0);
...
router.get("/operators/:id", async (req, res) => {
  try {
    ...
    res.json({
      ...
      reviews_summary: reviews.length > 0 ? {
        average_rating: Math.round(avgRating * 10) / 10,
        total_reviews: totalReviews,
        authenticity_score: reviews.reduce((s, r) => s + parseFloat(r.authenticityScore ?? "0.8"), 0) / reviews.length,
        sources: reviews.map((r) => ({
          source: r.source,
          count: r.reviewCount,
          avg_rating: parseFloat(r.rating),
        })),
        sentiment_score: reviews.reduce((s, r) => s + parseFloat(r.sentiment ?? "0.7"), 0) / reviews.length,
        recency_score: 70,
      } : null,
