...
import { ChangeNotificationStatusUseCase, CreateNotificationUseCase, GetNotificationUseCase, ListNotificationsUseCase, UpdateNotificationUseCase } from "../../application/use-cases.js";
...
export function createRoutes(repository: NotificationRepository): readonly Route[] {
  const clock = () => new Date().toISOString();
  const createUseCase = new CreateNotificationUseCase(repository, clock);
  const updateUseCase = new UpdateNotificationUseCase(repository, clock);
  const changeStatusUseCase = new ChangeNotificationStatusUseCase(repository, clock);
  ...
  return [
    {
      method: "GET",
      path: "/notifications/capability",
      handler: async () => ({ service: "notifications", aggregate: "Notification", capability: "tenant notifications email delivery" })
    },
    {
      method: "POST",
      path: "/notifications",
