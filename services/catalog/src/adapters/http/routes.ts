...
import { ChangeCatalogItemStatusUseCase, CreateCatalogItemUseCase, GetCatalogItemUseCase, ListCatalogItemsUseCase, UpdateCatalogItemUseCase } from "../../application/use-cases.js";
...
export function createRoutes(repository: CatalogItemRepository): readonly Route[] {
  const clock = () => new Date().toISOString();
  const createUseCase = new CreateCatalogItemUseCase(repository, clock);
  const updateUseCase = new UpdateCatalogItemUseCase(repository, clock);
  const changeStatusUseCase = new ChangeCatalogItemStatusUseCase(repository, clock);
  const getUseCase = new GetCatalogItemUseCase(repository);
  const listUseCase = new ListCatalogItemsUseCase(repository);

  return [
    {
      method: "GET",
      path: "/catalog/capability",
      handler: async () => ({ service: "catalog", aggregate: "CatalogItem", capability: "destinations activities accommodations addons" })
    },
    {
      method: "POST",
      path: "/catalog",
