...
import { ChangeProfileStatusUseCase, CreateProfileUseCase, GetProfileUseCase, ListProfilesUseCase, UpdateProfileUseCase } from "../../application/use-cases.js";
...
export function createRoutes(repository: ProfileRepository): readonly Route[] {
  const clock = () => new Date().toISOString();
  const createUseCase = new CreateProfileUseCase(repository, clock);
  const updateUseCase = new UpdateProfileUseCase(repository, clock);
  const changeStatusUseCase = new ChangeProfileStatusUseCase(repository, clock);
  const getUseCase = new GetProfileUseCase(repository);
  const listUseCase = new ListProfilesUseCase(repository);

  return [
    {
      method: "GET",
      path: "/identity/capability",
      handler: async () => ({ service: "identity", aggregate: "Profile", capability: "profiles roles invitations authentication" })
    },
    {
      method: "POST",
      path: "/identity",
