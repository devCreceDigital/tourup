...
import { ChangeEnrollmentStatusUseCase, CreateEnrollmentUseCase, GetEnrollmentUseCase, ListEnrollmentsUseCase, UpdateEnrollmentUseCase } from "../../application/use-cases.js";
...
export function createRoutes(repository: EnrollmentRepository): readonly Route[] {
  const clock = () => new Date().toISOString();
  const createUseCase = new CreateEnrollmentUseCase(repository, clock);
  const updateUseCase = new UpdateEnrollmentUseCase(repository, clock);
  const changeStatusUseCase = new ChangeEnrollmentStatusUseCase(repository, clock);
  const getUseCase = new GetEnrollmentUseCase(repository);
  const listUseCase = new ListEnrollmentsUseCase(repository);

  return [
    {
      method: "GET",
      path: "/enrollments/capability",
      handler: async () => ({ service: "enrollments", aggregate: "Enrollment", capability: "public and manual enrollments health data exports" })
    },
    {
      method: "POST",
      path: "/enrollments",
