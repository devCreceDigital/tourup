...
import { ChangeAssistantSessionStatusUseCase, CreateAssistantSessionUseCase, GetAssistantSessionUseCase, ListAssistantSessionsUseCase, UpdateAssistantSessionUseCase } from "../../application/use-cases.js";
...
export function createRoutes(repository: AssistantSessionRepository): readonly Route[] {
  const clock = () => new Date().toISOString();
  const createUseCase = new CreateAssistantSessionUseCase(repository, clock);
  const updateUseCase = new UpdateAssistantSessionUseCase(repository, clock);
  const changeStatusUseCase = new ChangeAssistantSessionStatusUseCase(repository, clock);
  const getUseCase = new GetAssistantSessionUseCase(repository);
  const listUseCase = new ListAssistantSessionsUseCase(repository);

  return [
    {
      method: "GET",
      path: "/assistant/capability",
      handler: async () => ({ service: "assistant", aggregate: "AssistantSession", capability: "ai sessions messages leads trip plans" })
    },
    {
      method: "POST",
      path: "/assistant",
