...
import { ChangeItineraryStatusUseCase, CreateItineraryUseCase, GetItineraryUseCase, ListItinerarysUseCase, UpdateItineraryUseCase } from "../../application/use-cases.js";
...
export function createRoutes(repository: ItineraryRepository): readonly Route[] {
  const clock = () => new Date().toISOString();
  const createUseCase = new CreateItineraryUseCase(repository, clock);
  const updateUseCase = new UpdateItineraryUseCase(repository, clock);
  const changeStatusUseCase = new ChangeItineraryStatusUseCase(repository, clock);
  const getUseCase = new GetItineraryUseCase(repository);
  const listUseCase = new ListItinerarysUseCase(repository);

  return [
    {
      method: "GET",
      path: "/itineraries/capability",
      handler: async () => ({ service: "itineraries", aggregate: "Itinerary", capability: "itineraries days events versioning" })
    },
    {
      method: "POST",
      path: "/itineraries",
