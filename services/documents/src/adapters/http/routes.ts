...
import { ChangeTravelerDocumentStatusUseCase, CreateTravelerDocumentUseCase, GetTravelerDocumentUseCase, ListTravelerDocumentsUseCase, UpdateTravelerDocumentUseCase } from "../../application/use-cases.js";
...
export function createRoutes(repository: TravelerDocumentRepository): readonly Route[] {
  const clock = () => new Date().toISOString();
  const createUseCase = new CreateTravelerDocumentUseCase(repository, clock);
  const updateUseCase = new UpdateTravelerDocumentUseCase(repository, clock);
  const changeStatusUseCase = new ChangeTravelerDocumentStatusUseCase(repository, clock);
  const getUseCase = new GetTravelerDocumentUseCase(repository);
  const listUseCase = new ListTravelerDocumentsUseCase(repository);

  return [
    {
      method: "GET",
      path: "/documents/capability",
      handler: async () => ({ service: "documents", aggregate: "TravelerDocument", capability: "document upload review approval rejection" })
    },
    {
      method: "POST",
      path: "/documents",
