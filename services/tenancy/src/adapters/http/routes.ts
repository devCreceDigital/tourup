...
import { ChangeTenantStatusUseCase, CreateTenantUseCase, GetTenantUseCase, ListTenantsUseCase, UpdateTenantUseCase } from "../../application/use-cases.js";
...
export function createRoutes(repository: TenantRepository): readonly Route[] {
  const clock = () => new Date().toISOString();
  const createUseCase = new CreateTenantUseCase(repository, clock);
  const updateUseCase = new UpdateTenantUseCase(repository, clock);
  const changeStatusUseCase = new ChangeTenantStatusUseCase(repository, clock);
  const getUseCase = new GetTenantUseCase(repository);
  const listUseCase = new ListTenantsUseCase(repository);

  return [
    {
      method: "GET",
      path: "/tenancy/capability",
      handler: async () => ({ service: "tenancy", aggregate: "Tenant", capability: "tenant lifecycle preferences onboarding" })
    },
    {
      method: "POST",
      path: "/tenancy",
